...
export default function PostDetailPage() {
  ...
  useEffect(() => {
    if (post) {
      if (post.name_type & 1)
        setSelectedNameType(1); // 닉네임 사용 가능하면 기본값
      else if (post.name_type & 2) setSelectedNameType(2); // 익명만 가능하면 기본값
    }
  }, [post]);

  // 투표 후 게시물 상태 업데이트 함수 (로컬 계산 방식)
  const updatePostAfterVote = (
    action: "vote_positive" | "vote_negative" | "vote_cancel",
  ) => {
    setPost((prev) => {
      if (!prev) return null;
      ...
      const updateCommentInList = (comments: any[]): any[] => {
        return comments.map((comment) => {
          ...
          if (comment.comments && comment.comments.length > 0) {
            return {
              ...comment,
              comments: updateCommentInList(comment.comments),
            };
          }
          return comment;
        });
  ...
  const article_positive_vote_handler = async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "투표 처리 중 오류가 발생했습니다.";
      alert(errorMessage);
  ...
  const comment_negative_vote_handler = async (commentId: number) => {
    try {
      ...
      const action =
        comment.my_vote === false ? "vote_cancel" : "vote_negative";
      await voteComment(commentId, action);
      updateCommentAfterVote(commentId, action);
    } catch (error: any) {
      console.error("댓글 투표 오류:", error);
  ...
  const handleScrap = async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "스크랩 처리 중 오류가 발생했습니다.";
      alert(errorMessage);
    }
  };
  ...
  const BookmarkIcon = ({ isFilled }: { isFilled: boolean }) => {
    ...
    return (
      ...
      >
        <path
          d="M12.6666 14.5L7.99992 11.1667L3.33325 14.5V3.83333C3.33325 3.47971 3.47373 3.14057 3.72378 2.89052C3.97382 
        2.64048 4.31296 2.5 4.66659 2.5H11.3333C11.6869 2.5 12.026 2.64048 12.2761 2.89052C12.5261 3.14057 
        12.6666 3.47971 12.6666 3.83333V14.5Z"
          stroke={color}
          strokeWidth="1.33333"
          strokeLinecap="round"
    ...
    <div className="flex flex-col items-center bg-white md:p-8 p-4 w-full min-h-screen">
      <div className="sm:w-[70vw] w-full max-w-7xl">
        ...
        <div className="flex flex-col gap-[8px]">
          <div className="text-[18px] font-bold text-black leading-[25.2px]">
            {post.title}
          </div>
          ...
          {post.metadata && (
            <div className="mt-1">
              ...
              {post.metadata.expire_at &&
                typeof post.metadata.expire_at === "string" && (
          ...
          <div className="flex flex-row w-full h-fit justify-between items-center">
            <div
              ...
              onClick={() => {
                if (post.name_type === 1)
                  router.push(`/profile/${post.created_by.id}`);
              }}
            >
              <img
        ...
        <div className="flex flex-col items-end">
          <div className="flex justify-end relative">
            <span
              ...
              onClick={() =>
                mappedAttachments.length > 0 && setPopoverOpen((o) => !o)
              }
            >
              ...
              {post.is_mine ? (
                <>
                  ...
                  <button
                    ...
                    className="p-2 flex flex-row gap-2 cursor-pointer border border-[#E9E9E9] text-[#ED3A3A] rounded text-sm leading-snug hover:bg-gray-100 transition"
                  >
                    삭제
                  </button>
                </>
                ...
                <>
                  ...
                  {/* 담아두기 버튼 */}
                  <div
                    className={`p-2 flex flex-row gap-2 cursor-pointer border rounded text-sm leading-snug hover:shadow-md transition ${
                      post.my_scrap
                        ? "border-[#ED3A3A] bg-[#ed3a3a] text-white"
        ...
        <div className="flex flex-col gap-1 mt-4">
          {post.my_comment_profile && (
            <div className="flex flex-row gap-2 items-center px-2 mb-2">
              <img
                src={post.my_comment_profile.profile.picture}
                alt="my profile"
                width={24}
                height={24}
              ...
              <span className="font-medium text-md">
                {post.my_comment_profile.profile.nickname}
              </span>
            </div>
          )}
          <ReplyEditor

...
import {
  makeMarketMetadata,
  makePosterMetadata,
} from "@/lib/utils/article_metadata";
...
export default function Write() {
  ...
  const isPosterBoard =
    boards.find((b) => b.id === boardId)?.ko_name === "포스터";
  ...
  const handleSavePost = async () => {
    ...
    try {
      if (isEditMode && editPostId) {
        ...
        alert("글이 수정되었습니다.");
        (window as any).FlutterChannel?.postMessage("PostWritePageExit");
      } else {
        // 생성 모드
        const newArticle = {
          title,
        ...
        await createPost({ boardId: boardId!, newArticle });
        alert("글이 저장되었습니다.");
        (window as any).FlutterChannel?.postMessage("PostWritePageExit");
      }
    } catch (err) {
      ...
      <div className="sm:w-[70vw] w-full max-w-7xl">
        ...
        <PostOptionBar
          ...
          onChangeBoard={(id) => {
            ...
            const market =
              !!board && /장터|거래|마켓/i.test(board.ko_name ?? "");
            setIsMarket(market);
          }}
          onChangeCategory={(id) => {
                ...
                <Calendar
                  ...
                  onDateClick={(date) => {
                    ...
                    if (selected >= tomorrow && selected <= maxDate) {
                      ...
                      alert(
                        "만료일은 최소 내일, 최대 60일 뒤까지 설정 가능합니다.",
                      );
                    }
                  }}
                />

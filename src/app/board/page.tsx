...
export default function Board() {
  ...
  const [isTimeAbsoulte, setIsTimeAbsolute] = useState(true);
  ...
  const renderBoard = () => {
    ...
    if (currentBoardId === 4 && isGrid) {
      return <MarketArticleContainer />;
    } else if (currentBoardId === 2 && isTimeAbsoulte) {
      return (
        <BoardArticleList
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between mt-2 sm:mt-0 flex-wrap">
                  ...
                  {currentBoardId === 2 && (
                    <label className="flex items-center gap-1.5 mr-2 text-md text-gray-600 cursor-pointer select-none ml-auto mt-2 sm:mt-0">
                      <input
                        type="checkbox"
                        checked={isTimeAbsoulte}
                        onChange={(e) => setIsTimeAbsolute(e.target.checked)}
                        className="accent-[#e15858] w-3.5 h-3.5 cursor-pointer"
                      />
                      날짜로 보기
                    </label>
                  ...
                  {currentBoardType === "all" && (
                    <label className="flex items-center gap-1.5 text-md text-gray-600 cursor-pointer select-none ml-auto mt-2 sm:mt-0">
                      <input
                        ...
                        onChange={(e) =>
                          setExcludePortalNotice(e.target.checked)
                        }
                        className="accent-[#e15858] w-3.5 h-3.5 cursor-pointer"
                      />
                      포탈 공지 제외
                ...
                <div className="max-w-none">
                  {currentBoardType === "all" && (
                    <BoardAllArticleList
                      query={search}
                      hidePortalNotice={excludePortalNotice}
                    />
                  )}
                  {currentBoardType === "popular" && (
                    <BoardHotArticleList query={search} />
                  )}
                  {currentBoardType === "board" && renderBoard()}
                </div>
              </div>
            </div>
          </div>

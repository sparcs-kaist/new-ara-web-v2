...
export default function NavBarMiddle() {
  ...
  const [pinnedItem, setPinnedItem] = useState<string | null>(null); // 클릭으로 고정될 드롭다운 메뉴
  ...
  const handleMouseEnter = (menu: string) => {
    if (!pinnedItem) setHoveredItem(menu);
  };
  ...
  const handleToggleClick = (menu: string) => {
    setPinnedItem(prev => (prev === menu ? null : menu));
    setHoveredItem(null);
  };
  ...
  const openMenu = pinnedItem ?? hoveredItem;

  return (
    <div ref={containerRef} className="hide-below-900 flex gap-[32px] font-medium">
      {/* 자유게시판/전체보기 */}
      <Link
      ...
      >
        <div
          className="py-2 whitespace-nowrap cursor-pointer flex items-center gap-3 hover:text-ara_red transition-colors duration-200"
          onClick={() => handleToggleClick("news")}
          role="button"
        >
          소식
          <DropdownArrow isOpen={openMenu === "news"} isHovered={hoveredItem === "news"} />
        </div>
        {openMenu === "news" && (
      ...
      >
        <div
          className="py-2 whitespace-nowrap cursor-pointer flex items-center gap-3 hover:text-ara_red transition-colors duration-200"
          onClick={() => handleToggleClick("market")}
          role="button"
        >
          거래
          <DropdownArrow isOpen={openMenu === "market"} isHovered={hoveredItem === "market"} />
        </div>
        {openMenu === "market" && (
      ...
      >
        <div
          className="py-2 whitespace-nowrap cursor-pointer flex items-center gap-3 hover:text-ara_red transition-colors duration-200"
          onClick={() => handleToggleClick("communication")}
          role="button"
        >
          소통
          <DropdownArrow isOpen={openMenu === "communication"} isHovered={hoveredItem === "communication"} />
        </div>
